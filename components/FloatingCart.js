...
export default function FloatingCart(props){
    ...
    return(
        <div id='floating-cart'
             className={props.expanded ? 'inset-x-0 bottom-4 sticky w-2/3 bg-white p-4 shadow-2xl rounded-2xl h-1/2 m-auto' :
                 'inset-x-0 bottom-4 sticky w-2/3 bg-white p-4 shadow-2xl rounded-full h-1/2 m-auto'
             }>
            ...
            <div>
                {props.expanded ? <table className='w-full text-left font-default'>
                    ...
                    {props.cartItems.map(cartItem => {
                        return <ProductRow key={cartItem.id} name={cartItem.name} price={cartItem.price}
                                           qty={cartItem.qty} handleRemoveItem={props.handleRemoveItem}
                                           cartId={cartItem.cartId} selectionChoice={cartItem.selectionChoice}
                    ...
                    {props.expanded && <div className='flex justify-end mx-4 mt-2 font-default font-bold'>
                        {props.subtotal > 0 && <h1 className='text-xl'>Subtotal ${props.subtotal}</h1>}
                    </div>}
            </div>
        </div>
    )
