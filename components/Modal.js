...
export default function Modal(props) {
    ...
    const renderIngredients = props.ingredientSelectionState.map(ingredient=>{
        return(
            <IngredientSelector
                key={ingredient.name}
                id={ingredient.id}
    ...
    return (
        <div>
            <div id='top-row' className='flex justify-between mb-6'>
                ...
                <button onClick={props.closeModal} className='text-4xl'>x</button>
            </div>
            ...
            <div id='lower-block' className='flex justify-between mt-6'>
                ...
                <h1 className='text-3xl font-default text-right'>${props.price}</h1>
            </div>
